	...
	public static void generateReport(List<ModelPost> posts)
	{
		...
		for (ModelPost post:posts) {
			...
			if (!post.getType().equals("Emergency")
				&& !post.getType().equals("Electrical Complaints")
				&& !post.getType().equals("Public Incidents")
				&& !post.getType().equals("Household Concerns")
				&& !post.getType().equals("Road Complaints"))
			{
				continue;
			}
